...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 25 10:35:12 2020
...
# ML0 = ML_TF(obs_ind = inp_LFtrain, obs = (y_HF_LFtrain), amp_init=1., len_init=1., var_init=1., num_iters = 100000)
# amp0, len0, var0 = ML0.GP_train()
ML0 = ML_TF(obs_ind = Norm1(inp_LFtrain,inp_LFtrain), obs = Norm1(y_HF_LFtrain,y_HF_LFtrain), amp_init=1.0, len_init=1.0, var_init=1.0, num_iters = 1000)
amp0, len0, var0 = ML0.GP_train()
...
for ii in np.arange(0,Ninit_GP,1):
    ...
    y_LF_GP = np.concatenate((y_LF_GP, np.array(InvNorm1(np.mean(np.array(samples0),axis=0),y_HF_LFtrain)).reshape(1)))
    y_HF_GP = np.concatenate((y_HF_GP, np.array((LS1.Scalar_Borehole_HF_nD(inpp))).reshape(1)))
    y_GPtrain = np.concatenate((y_GPtrain, (np.array((LS1.Scalar_Borehole_HF_nD(inpp))-np.array(InvNorm1(np.mean(np.array(samples0),axis=0),y_HF_LFtrain)))).reshape(1)))
...
ML = ML_TF(obs_ind = Norm1(inp_GPtrain,inp_GPtrain), obs = Norm1(y_GPtrain,y_GPtrain), amp_init=1., len_init=1., var_init=1., num_iters = 1000)
...
for ii in np.arange(0,Nsub,1):
    ...
    GP_diff = np.mean(InvNorm1(np.array(samples1),y_GPtrain),axis=0)
    ...
    if u_check > u_lim:
        ...
        ML = ML_TF(obs_ind = Norm1(inp_GPtrain,inp_GPtrain), obs = Norm1(y_GPtrain,y_GPtrain), amp_init=amp1, len_init=len1, var_init=var1, num_iters = Iters)
        amp1, len1, var1 = ML.GP_train()
