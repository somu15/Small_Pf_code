...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec 16 09:52:32 2020
...
Iters = 300
Ninit_GP = 12
lhd1 = lhs(2, samples=Ninit_GP, criterion='maximin')
lhd =  norm().ppf(lhd1)
...
y_HF_GP = np.array(Convert(LS1.Scalar_LS1_HF_2D(inp_GPtrain)))
...
for ii in np.arange(0,Nsub,1):
    ...
    if u_check > u_lim:
        ...
        K = np.array(ML.GP_predict(amplitude_var = amp1, length_scale_var=len1, observation_noise_variance_var=var1, pred_ind = Norm1(inp_GPtrain,inp_GPtrain), num_samples=num_s))
        GP_dep = np.concatenate((GP_dep, np.array(ML.GP_dependencies(Samples=K, LF=y_LF_GP)).reshape(1)))
