...
for ii in np.arange(0,200,1):
    N_doe = 1000
    inp_doe = DR1.BoreholeRandom(N=N_doe)
    y_LF_doe = ML0.DNN_pred(inp_LFtrain,y_HF_LFtrain,DNN_model,Ndim,inp_doe).reshape(N_doe)
...
for kk in np.arange(1,Nlim,1):
    ...
    for ii in np.arange(0,Nsub,1):
        ...
        LF = ML0.DNN_pred(inp_LFtrain,y_HF_LFtrain,DNN_model,Ndim,inpp).reshape(1)
        samples1 = ML.GP_predict(amplitude_var = amp1, length_scale_var=len1, pred_ind = Norm1(inpp,P,Ndim), num_samples=num_s)
        mean_sto = InvNorm3(np.mean(np.array(samples1),axis=0),y_GPtrain)
        std_sto = np.std(InvNorm3(np.array(samples1),y_GPtrain),axis=0)

        if mean_sto != 0 or std_sto != 0:
            GP_diff = mean_sto
            additive = y1_lim[kk-1]
            u_check = (np.abs(LF + GP_diff - additive))/std_sto
