...
inp_GPtrain = lhd
inp_GPtrain0 = lhd0
y_LF_GP = LS1.Triso_1d(inp_GPtrain0)
y_HF_GP = LS1.Triso_2d(inp_GPtrain)
y_GPtrain = y_HF_GP - y_LF_GP
ML = ML_TF(obs_ind = Norm1(inp_GPtrain0,inp_GPtrain0, Ndim), obs = Norm3(y_GPtrain,y_GPtrain))
...
for ii in np.arange(0,Nsub,1):
    ...
    if u_check > u_lim:
        ...
        y1[ii,0] = np.array((LS1.Triso_2d(inp_HF[None,:]))).reshape(1)
...
for kk in np.arange(1,Nlim,1):
    ...
    for ii in np.arange(0,(Nsub),1):
        ...
        for jj in np.arange(0,Ndim,1):
            ...
            inpp[0,jj] = nxt[0,jj]
        LF = LS1.Triso_1d(inpp.reshape(1,Ndim))
        ...
        if u_check > u_lim: 
            ...
            y_nxt = np.array((LS1.Triso_2d(inpp[0,ind_HF].reshape(1,Ndim0)))).reshape(1)
            inp_GPtrain0 = np.concatenate((inp_GPtrain0, inpp.reshape(1,Ndim)))
            y_LF_GP = np.concatenate((y_LF_GP, np.array(LF).reshape(1)))
