...
DNN_model = ML0.DNN_train(dim=Ndim, seed=100, neurons1=8, neurons2=4, learning_rate=0.002, epochs=5000)
...
counter = 1
file1 = open('/home/dhullaks/projects/Small_Pf_code/src/1D_TRISO_GP_DNN1/Results.csv','w')
...
for ii in np.arange(0,Nsub,1):
    ...
    # u_check = (np.abs(LF + GP_diff - additive))/InvNorm3(np.std(np.array(samples1),axis=0),y_GPtrain)
    u_GP[ii,0] = u_check
    ...
    if u_check > u_lim:
        ...
        # DNN_model = ML0.DNN_train(dim=Ndim, seed=100, neurons1=6, neurons2=4, learning_rate=0.005, epochs=5000)
        subs_info[ii,0] = 1.0
    file1 = open('/home/dhullaks/projects/Small_Pf_code/src/1D_TRISO_GP_DNN1/Results.csv','r')
...
Nretrain = 12
inp_GPtrain = np.zeros((Nretrain,Ndim))
y_GPtrain = np.zeros(Nretrain)
...
for kk in np.arange(1,Nlim,1):
    ...
    for ii in np.arange(0,(Nsub),1):
        ...
        if ii < Nretrain:
            ...
            ML = ML_TF(obs_ind = Norm1(inp_GPtrain,inp_GPtrain,Ndim), obs = Norm3(y_GPtrain,y_GPtrain))
            amp1, len1 = ML.GP_train(amp_init=1.0, len_init=1.0, num_iters = 1000)
        else:
            LF = ML0.DNN_pred(inp_LFtrain,y_HF_LFtrain,DNN_model,Ndim,inpp)[0]
...
os.chdir('/home/dhullaks/projects/Small_Pf_code/src/1D_TRISO_GP_DNN1')
