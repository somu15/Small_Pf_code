...
class LimitStateFunctions:
    ...
    def Triso_2d_norm(self, Input_vec=None):

        siz1 = len(Input_vec[:,0])
        out1 = np.zeros(siz1)
        TM1 = TM()
        Input_vec_norm = np.zeros((siz1,12))
        ...
        rv6 = norm(loc=1900.0,scale=20.0) # ipyc_d
        rv7 = norm(loc=1900.0,scale=20.0) # opyc_d
        rv8 = norm(loc=1.05,scale=0.005) # ipyc_b
        rv9 = norm(loc=1.05,scale=0.005) # opyc_b
        rv10 = uniform()
        ...
        Input_vec_norm[:,6] = rv7.ppf(rv_norm.cdf(Input_vec[:,6]))
        Input_vec_norm[:,7] = rv8.ppf(rv_norm.cdf(Input_vec[:,7]))
        Input_vec_norm[:,8] = rv9.ppf(rv_norm.cdf(Input_vec[:,8]))
        Input_vec_norm[:,9] = rv10.ppf(rv_norm.cdf(Input_vec[:,9]))
        # Input_vec_norm[:,10] = rv10.ppf(rv_norm.cdf(Input_vec[:,10]))
        # Input_vec_norm[:,11] = rv10.ppf(rv_norm.cdf(Input_vec[:,11]))

        for ii in np.arange(0,siz1,1):
            out1[ii] = TM1.Triso_2d(Input_vec_norm[ii,0],Input_vec_norm[ii,1],Input_vec_norm[ii,2],Input_vec_norm[ii,3],Input_vec_norm[ii,4],Input_vec_norm[ii,5],Input_vec_norm[ii,6],Input_vec_norm[ii,7],Input_vec_norm[ii,8],Input_vec_norm[ii,9]) # ,Input_vec_norm[ii,10],Input_vec_norm[ii,11])
    ...
    def Triso_1d_norm(self, Input_vec=None):
        ...
        for ii in np.arange(0,siz1,1):
            out1[ii] = TM1.Triso_1d(Input_vec_norm[ii,0],Input_vec_norm[ii,1],Input_vec_norm[ii,2],Input_vec_norm[ii,3],Input_vec_norm[ii,4],Input_vec_norm[ii,5],Input_vec_norm[ii,6],Input_vec_norm[ii,7],Input_vec_norm[ii,8],Input_vec_norm[ii,9],Input_vec_norm[ii,10],Input_vec_norm[ii,11]) # )
