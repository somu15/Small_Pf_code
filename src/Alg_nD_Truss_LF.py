...
os.chdir('/Users/dhulls/projects/Small Pf/Small_Pf_code/src')
...
#     inp = (DR1.TrussRandom())
#     inpp = inp[None,:]
#     y1[ii,0] = np.array(LS1.Truss_HF(inpp))
#     inp1[ii,:,0] = inp
...
#             rv1 = norm(loc=inp1[ind_max,jj,kk],scale=1.0)
#             prop = (rv1.rvs())
#             r = np.log(DR1.TrussPDF(rv_req=prop, index=jj)) - np.log(DR1.TrussPDF(rv_req=(inp1[ind_max,jj,kk]),index=jj)) # rv.pdf((prop))/rv.pdf((inp1[ii-(int(Psub*Nsub)),jj,kk]))
...
#             inpp[jj] = nxt[0,jj]
#         y_nxt = np.array(LS1.Truss_HF(inpp[None,:])).reshape(1)
...
inp_GPtrain = lhd
y_LF_GP = LS1.Truss_LF(inp_GPtrain)
...
prop_std = np.array([1.0,1.0,1.0,0.25,0.1])
...
# req = 7.2761e-05 (5000 sims per subset and 4 subsets; Num HF = 105)