...
#             prop = np.exp(rv1.rvs())
#             r = np.log(DR1.FluidPDF(rv_req=(prop), index=jj)) - np.log(DR1.FluidPDF(rv_req=(inp1[ind_max,jj,kk]),index=jj)) # rv.pdf((prop))/rv.pdf((inp1[ii-(int(Psub*Nsub)),jj,kk]))
...
for kk in np.arange(1,Nlim,1):
    ...
    for ii in np.arange((int(Psub*Nsub)),(Nsub),1):
        ...
        for jj in np.arange(0,Ndim,1):
            ...
            prop = np.exp(rv1.rvs())
            r = np.log(DR1.HolesPDF(rv_req=prop, index=jj)) - np.log(DR1.HolesPDF(rv_req=(inp1[ind_max,jj,kk]),index=jj)) # np.log(rv.pdf((prop)))-np.log(rv.pdf((inp1[ind_max,jj,kk])))
            if r>np.log(uni.rvs()):
                nxt[0,jj] = prop
            else:
                nxt[0,jj] = inp1[ind_max,jj,kk]
...
os.chdir('/home/dhullaks/projects/Small_Pf_code/src/Holes_Alg')
