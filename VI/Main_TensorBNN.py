...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec 21 21:15:16 2020
...
pred = predictor("TrigRegression/", dtype = dtype )

pred_input = np.linspace(-2,2,num=400).reshape(400,1)
predictions = pred.predict(pred_input,n=1)


out_mean = np.mean(predictions,axis=0).reshape(400)
out_std = np.std(predictions,axis=0).reshape(400)
in_data = np.linspace(-2,2,num=400)
plt.plot(in_data,out_mean,in_data,out_mean-out_std,in_data,out_mean+out_std)
