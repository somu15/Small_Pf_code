...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 29 15:55:12 2020
...
def bnn_log_prob_fn(X, y, params, get_mean=False):
    ...
    """
    # tf.concat([tf.reshape(g, [-1]) for g in tf.gradients(t, tf.trainable_variables())], axis=0)
    ...
    # labels_dist = net(X)
    # if get_mean:
    #     return tf.reduce_mean(tf.concat([tf.reshape(g, [-1]) for g in labels_dist.log_prob(y)], axis=0))
    # return tf.reduce_sum(tf.concat([tf.reshape(g, [-1]) for g in labels_dist.log_prob(y)], axis=0))

def prior_log_prob_fn(w_prior, b_prior, params):
    log_prob = 0
    # print(log_prob)
    for w, b in params:
        # tf.print(w_prior.log_prob(w))
        log_prob += tf.reduce_sum(w_prior.log_prob(w))
        log_prob += tf.reduce_sum(b_prior.log_prob(b))
    return log_prob
    
    # for w, b in params:
    #     log_prob += tf.reduce_sum(tf.concat([tf.reshape(g, [-1]) for g in w_prior.log_prob(w)], axis=0)) # 
    #     log_prob += tf.reduce_sum(tf.concat([tf.reshape(g, [-1]) for g in b_prior.log_prob(b)], axis=0)) # 
    # return log_prob
